...
`docs/research/research_log.md` before any collection. Fisher exact tests are two-sided on identical seeds and conditions.

## 1. Previous closed-loop diagnostic (summary of `docs/reports/01_closed_loop_diagnostics.md`)
- Expert replay: 10/10.
...
## 20. Reproduction
```
scripts/experiments/run_closed_loop_diagnostics.sh                       # A/BC baseline sweep (resumable)
scripts/experiments/run_corrective_phase.sh                              # collect B, C; train B, C, Bm; evaluate (resumable)
scripts/experiments/run_phase5_remaining.sh                              # C40k evaluation + full recovery benchmark
# C40k training:
...
## 21. Critical artifacts to back up (gitignored; hashes in `docs/artifact_manifest.json`)
...
Results will be appended to `docs/research/research_log.md` and a Phase 6 report. Only if memorised scenes become reliable should the 80-demo held-out
phase start.
