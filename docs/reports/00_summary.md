...
Model size never changed (≈2.01M trainable). Every experiment was pre-registered in `docs/research/research_log.md` before its result. Detailed
reports: `docs/reports/01_closed_loop_diagnostics.md`, `docs/reports/02_corrective_data.md`, `docs/reports/03_failure_isolation.md`. Numbers are successes on identical seeds
...
## Reproduction
`scripts/experiments/run_closed_loop_diagnostics.sh`, `scripts/experiments/run_corrective_phase.sh`, `scripts/experiments/run_phase5_remaining.sh`, `scripts/experiments/run_phase6.sh`,
`scripts/experiments/run_phase6c.sh`, `scripts/experiments/run_phase7a.sh`, `scripts/experiments/run_phase7b.sh`, `scripts/experiments/run_phase7c.sh`, `scripts/experiments/run_phase8.sh`,
`scripts/experiments/run_phase8b.sh`. Then `.venv/bin/python -m evaluation.phase5_analysis`. Back up `artifacts/` (hashes in `docs/artifact_manifest.json`).
